...
from apps.core.authentication.services import generate_user_jwt_token, validate_staff_jwt_token
...
class TestStaffAuthContract:
    ...
    @override_settings(JWT_SECRET_KEY='a-jwt-secret-distinct-from-django-secret-key')
    def test_validate_token_uses_jwt_secret_key_for_verification(self, staff_user):
        """
        Regression test for Fase 0 / Tarea 6: signing and verification must share the
        same secret. With JWT_SECRET_KEY overridden to a value distinct from
        SECRET_KEY, a token generated by generate_user_jwt_token (which signs with
        JWT_SECRET_KEY) must still validate successfully via
        validate_staff_jwt_token (which must decode with JWT_SECRET_KEY too, not
        SECRET_KEY). Before the fix, validate_staff_jwt_token decoded with
        settings.SECRET_KEY only, so this would fail with an invalid-signature error.
        """
        token = generate_user_jwt_token(staff_user)
        ...
        assert data.get('valid') is True
        assert data.get('user', {}).get('id') == staff_user.id
